import {Connection, PublicKey,Keypair,ComputeBudgetProgram, SystemProgram, Transaction}from "@solana/web3.js";
import { Program, BN, AnchorProvider } from "@coral-xyz/anchor";
import { RaydiumCpSwap } from "../../idl/types/raydium_cp_swap";
import * as anchor from '@coral-xyz/anchor';


import {getAmmConfigAddress,
    getPoolAddress,
    getAuthAddress,
...
export class PoolService {
    ...
    async preparePoolAddresses(
        ...
        try {
            ...
            const [poolAuthority] = await getAuthAddress(
                this.program.programId
            );
    ...
    async createPool({
        ...
        try {
            ...
            if (ammConfigAddress) {
                ...
                poolAddresses = await this.preparePoolAddresses(
                    ammConfigIndex,
                    token0Mint,
                    token1Mint
                );
            ...
            const ammConfigExists = await this.accountExists(this.connection, poolAddresses.ammConfigAddress);
            if (!ammConfigExists) {
