...
import { Connection, PublicKey, Keypair, Transaction, SystemProgram, ComputeBudgetProgram, VersionedTransaction } from '@solana/web3.js';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!token0Mint || !token1Mint || !token0Account || !token1Account || 
        !initAmount0 || !initAmount1 || !creatorPublicKey || !ammConfigAddress) {
    ...
    const [vault0] = await PublicKey.findProgramAddress(
      [Buffer.from("pool_vault"), poolKeypair.publicKey.toBuffer(), new PublicKey(token0Mint).toBuffer()],
      programId
    );
    
    const [vault1] = await PublicKey.findProgramAddress(
      [Buffer.from("pool_vault"), poolKeypair.publicKey.toBuffer(), new PublicKey(token1Mint).toBuffer()],
      programId
    );
